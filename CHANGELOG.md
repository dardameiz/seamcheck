...
CLI/MCP surface didn't answer before: not "what changed since a ref" (`diff`/`check`) but
"what does the page I'm touching right now look like" - plus a batch of fixes found by
actually driving all of it against a real 500k-line project, not synthetic fixtures.

- **Added** - `seamcheck scope commit`/`scope push` (and the `seamcheck_scope` MCP tool):
  resolves the page(s) a staged commit or an unpushed push touches from the live git diff,
  and reports what's currently unresolved/unused there - a pre-existing issue on that page
  counts, not just what the diff introduced, unlike `diff`/`check`. `--serve` opens one
  map per touched page, already focused on it, instead of the whole-repo map with a picker.
- **Added** - `seamcheck install-hooks`: writes advisory `pre-commit`/`pre-push` hooks that
  run `scope` and print a short summary. Never blocks - always exits 0, whatever it finds.
- **Added** - F37: a build-graph divergence check (`build_gap`) - a module reachable from
  an entry point in source but silently absent from the bundler's own manifest, the shape
  a real incident took (a dynamic-imported chunk a build step dropped). Follows
  `import()`, not just static `import`, which the reachability walk was blind to before.
- **Fixed** - git hooks installed by `install-hooks` shelled out to a bare `python3 -m
  seamcheck.hooks`, so a `python3` with no seamcheck installed silently did nothing, and
  `-m` prepending the hook's CWD to `sys.path` let a project directory literally named
  `seamcheck` shadow the real package. Hooks now embed `sys.executable` and run the module
  by its own absolute path.
- **Fixed** - `install-hooks` assumed hooks live at `.git/hooks/` - wrong for a project
  using husky, lefthook or any custom `core.hooksPath` (common in a mixed JS/Python repo):
  real, marker-carrying hook files got written and git never read them. Resolved via
  `git rev-parse --git-path hooks` now (also correct for a worktree or a submodule); a
  husky-managed directory is never written into at all, since it is regenerated by `npm
  install` - the exact lines to add to `.husky/pre-commit`/`.husky/pre-push` are printed
  instead.
- **Fixed** - a hook run imported and scanned the host project without suppressing its
  own start-up logging, so a WARNING line written to the host's own log file on every
  single run looked, to the scan cache's freshness check, like the newest input on every
  run - a ~12s cache hit became a ~60-70s cold scan on every commit and push.
  `scoped_findings()` also checked the git diff AFTER scanning; reordered so nothing
  staged/unpushed skips the scan entirely.
- **Fixed** - `check --format json` fell into the whole-graph JSON renderer, which hit its
  own size gate on any real project (75 MB / ~18.7M tokens measured) and exited 3 with the
  CI verdict never computed, undocumented in `help check`. Now prints the actual verdict
  (`passed`, `new_unresolved`, `counts`, ...) as JSON - the same dict the MCP
  `seamcheck_check` tool already returns.
- **Fixed** - a one-off script a project's own `.gitignore` disowns (`OTHER/`, by
  convention) fed both the JS reachability walk and the scan cache's freshness check as if
  it were the product - a Playwright probe living there was read as a first-party writer
  of a DOM element it never touches in production. New `seamcheck.gitfiles.tracked_files`
  asks `git ls-files` directly rather than re-parsing `.gitignore` by hand, so nested
  `.gitignore` files and negation are honoured for free.
- **Fixed** - six false-positive classes found by hand-checking 110 live findings on a
  real project (26 wrong): `setAttribute`/`removeAttribute`/`toggleAttribute` (and a name
  held in a `const`) now count as producers, not phantom unresolved reads; a CSS-escaped
  character in a class name (`.p-0\.5`, how Tailwind spells a fractional class) is
  unescaped instead of truncating the token; a descendant/combinator selector's WRITE
  attributes only to its last compound, not every ancestor in the chain - corrected
  same-day after re-measurement caught it silently dropping the ancestor as evidence too
  (`.goal-bar .progress` with zero `.goal-bar` producers anywhere is still a real finding;
  only the WRITE target narrows, existence-checking does not).
- **Fixed** - the served scope map's "view code" panel could never fetch a file's real
  contents, even though the map genuinely was being served - the server endpoint that
  serves source refuses any path not on an explicit allow-list, and the scoped map's
  lighter renderer never populated one. Found live, driving a real map on a real project.
- **Changed** - `serve` is `map` under another name and always has been - one
  implementation, so it no longer takes its own line in `seamcheck --help` or
  `docs/commands.md`'s command table. `seamcheck serve` still works exactly as before.

**Measured:** precision **54%** (133 hand-labelled claims) · recall **6/6** · render
**46/46**. Coverage is unchanged by construction - the F44 fixes reclassify existing
`unresolved` findings as `connected`, they do not change what gets judged at all - and the
corpus was re-run to confirm: 34/34 cloned repos scanned clean, no CRASH, no repo lost its
routes, no IMPLAUSIBLE volume.
