# Signal Map — instructions for an AI agent

Signal Map is a static-analysis tool that maps what a Django + JavaScript project connects
...
everything Signal Map found", ask them whether they mean the `unused` and `unresolved`
...
```bash
python manage.py dump_connectivity_map                       # scan + human summary
...
- `signal_map_check(repo_root)` — `{passed, new_unresolved, new_unused, triage_invalidated, counts}`
- `signal_map_report(fmt, repo_root)` — the rendered digest (`terminal`, `markdown`, `html`)
- `signal_map_explain(symbol_id, repo_root)` — one symbol's evidence as markdown
- `signal_map_triage(symbol_id, status, repo_root, reason)` — record a disposition
