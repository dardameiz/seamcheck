...
# husky v9+'s own convention: `.husky/_` is git's `core.hooksPath`, but the directory is
# regenerated by `npm install` (via `husky` in a `prepare` script) - a file written
# there directly is silently discarded the next time anyone installs. Matched by a path
# SEGMENT, not a suffix, so both `.husky/_` and a nested repo's `sub/.husky/_` are caught.
_HUSKY_SEGMENT = os.path.join(".husky", "_")


def _git_hooks_dir(repo_root: str) -> str | None:
    """Where git will actually look for hooks to run - not always `.git/hooks`.

    `git rev-parse --git-path hooks` is git's own answer, and respects `core.hooksPath`
    (husky, lefthook, or any custom setup - common in a mixed JS/Python repo, which is
    exactly this project's shape), worktrees (hooks live in the MAIN checkout, not the
    worktree) and submodules (hooks live in the superproject's `.git/modules/<name>/`,
    not a `.git/` the submodule directory doesn't have). `install_hooks()` used to
    hardcode `os.path.join(repo_root, ".git", "hooks")`, so on a project using husky
    (`core.hooksPath=.husky/_`) it wrote real, marker-carrying hook FILES that git never
    read - confirmed on pointlessbutton (T14): `.git/hooks/pre-commit` existed, was
    installed, and never once ran; a real commit produced no seamcheck output at all.

    `None` on any failure (not a git repo, git missing, an old git without
    `--git-path`) - the caller falls back to the old `.git/hooks` assumption rather than
    refusing outright, so this can only make the answer MORE accurate, never less.
    """
    try:
...
def _hook_script(mode: str) -> str:
    """The literal shell script `install_hooks()` writes into `<hooks-dir>/<name>`.
...
def install_hooks(repo_root: str = ".") -> str:
    """Write pre-commit and pre-push where GIT will actually look for them.

    Overwrites only what this function itself wrote before (a marker line, checked
    before overwriting anything - a hand-written hook already there is left alone and
    named in the returned message, never silently replaced).

    Resolved via `_git_hooks_dir` (git's own `core.hooksPath` answer), not assumed to be
    `.git/hooks` - see that function's docstring for why the assumption was wrong on
    this project specifically (T14: husky's `core.hooksPath=.husky/_`, real files
    written, git never reading them). A husky-managed directory is never written into at
    all - `.husky/_` is regenerated by `npm install`, so a file placed there directly is
    silently discarded the next time anyone installs - the caller is told to add the
    hook invocation to `.husky/pre-commit`/`.husky/pre-push` by hand instead, which
    husky's own regeneration does not touch.
    """
    hooks_dir = _git_hooks_dir(repo_root) or os.path.join(repo_root, ".git", "hooks")

    if hooks_dir.rstrip(os.sep).endswith(_HUSKY_SEGMENT):
        return (
            f"This project's hooks are managed by husky ({hooks_dir}, from git's own "
            "`core.hooksPath`) - regenerated by `npm install`, so a file seamcheck put "
            "there would be silently discarded on the next install. Add these lines to "
            "`.husky/pre-commit` and `.husky/pre-push` instead (create them if husky "
            "has not yet, and `chmod +x` them):\n"
            f'  "{sys.executable}" "{os.path.abspath(__file__)}" commit   # pre-commit\n'
            f'  "{sys.executable}" "{os.path.abspath(__file__)}" push     # pre-push'
        )
