# Extracting Seamcheck into its own repository

Seamcheck is developed as a self-contained Django app inside a host project, so it is
always exercised against a real codebase. Publishing it is a move, not a rewrite:

```bash
mkdir seamcheck
git -C seamcheck init
mv seamcheck seamcheck/
cd seamcheck
mv seamcheck/{pyproject.toml,LICENSE,README.md,CONTRIBUTING.md,CODE_OF_CONDUCT.md,SECURITY.md,PACKAGING.md} .
mv seamcheck/.github .
...
```
seamcheck/
  pyproject.toml      packages = ["seamcheck"]
  LICENSE  README.md  CONTRIBUTING.md  CODE_OF_CONDUCT.md  SECURITY.md
  .github/            issue + PR templates, dependabot, workflows
  seamcheck/         the package
```

Nothing in `seamcheck/` imports from the host project, and every project-specific path
comes from `SEAMCHECK_CONFIG` in the host's settings — so no source change is needed.
...
1. Copy `.github/workflows/seamcheck.yml` from the host repo (it lives at the host root
