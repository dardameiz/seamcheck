...
`pip install signal-map` puts the package in site-packages, where the parsers' `import
...
def run_parser(script: str, paths: list[str], what: str) -> list[str]:
    ...
    try:
        ...
        _report(what, "signal-map: no %s symbols - Node.js is not on PATH. Every other "
                      "extractor still ran.", what)
        return []
    if result.returncode != 0:
        _report(what, "signal-map: no %s symbols - %s exited %s. Every other extractor "
