...
**Landed (2026-09-03), narrower than drawn below:** `MapDocument` in `map_html.py` holds head / tail / chunks and decides late between `single_file()` (inline `text/plain` blocks, unchanged) and `bundle()` (`index.html` + `data/<chunk>.js` JSONP, `SC.chunk(name, enc, payload)`); `api.map_document()` / `api.write_map_document()` write either, auto-bundling past `SINGLE_FILE_LIMIT` (50 MB) with a note; `--bundle` on both CLIs; served mode serves the in-memory bundle under `/<token>/data/…` (bare token 301s to the slash form so relative `data/` resolves). No `map_bundle.py` and no separate manifest file: the manifest stays inline in `index.html`, but it is now counts only. **Second cut (2026-09-03, same day):** CONSOLE section rows → chunks `c<key>` (`_console_payload(console, chunks)`; sections keep `total`/`status_totals`, a section under the inline limit keeps its rows), OBSERVED boxes → `o<i>` for every page (`_observed_lazy`; `page`/`screenshot`/`count` stay), FILES tree + `filePage` → one `files` chunk (`_files_lazy`; `FILES` is `{count, chunk}` or `{count, list, filePage}`). JS: `withRows(sec, drawn)` / `withFiles(drawn)` / the `o.chunk` branch of `observedHtml()` draw a "Loading N…" gap, `readChunk` → set → `dropChunk` → re-render only if `mode` (and `observedAt`) is unchanged; `FILE_TOTALS` is filled by `indexFiles()` when the list lands, `bestPageFor` reads `FILES.filePage`. pointlessbutton index.html 2.2 MB → 386 KB (app JS 170 KB + CSS 69 KB + MAPDATA 92 KB, of which the `files` string table is 48 KB and scales with file count - the next thing to shard is that table per page, and the 55 MB search chunk). `tools/synth_map.py --bundle` measures the folder form; tests in `test_map_bundle.py` (`LazySections`) and `test_map_runs_in_a_browser.py` (`RowsArriveWhenOpened`, single file and bundle).
...
| AST memory | every tree of the repo held for the whole scan (n8n 3.7 GB RSS) | budgeted cache, ¼ RAM default, `SEAMCHECK_AST_CACHE_MB`; over-budget trees streamed per extractor; n8n at 1 GB budget: 1.4 GB RSS | `iter_parsed` generator; `_SELECTORS_CACHE` keyed on (path, stamp) instead of holding the tree; `extract_js_class_usages` / `extract_js_css_tokens` chain the parser stream instead of listing it |

Remaining in this phase, in order of what the numbers say: **fuse the per-tree readers into one pass** (above the budget each of `extract_dom_selectors` ×3, `extract_js_class_usages`, `extract_js_css_tokens`, `extract_js_dom_definitions` re-parses the remainder - the selectors step is 122 s of n8n's 325 s with a 1 GB budget; one walk per tree that feeds every reader would parse the over-budget files once), rendering 6.2 s, commit history 7.2 s in-run, JS modules 5.2 s, Redis 3.5 s. Map identity is checked by comparing `MAPDATA` and every chunk against the previous render (`pb_map7` vs `pb_map8`: 255/256 chunks identical; the `commits` chunk differed only in which commit is `head`, because the repository moved - and, until 5b, in the order of removed files, which was hash order).

Pre-existing failures logged as backlog while gating this phase (all fail on `db4eebf08` too): `test_mcp_server … four_tools_are_registered`, `test_meaning … specific_kind_beats_the_generic_status_text`, `test_pipeline_dom_css_regression … two_writer_element_is_flagged`, `test_pipeline_fixture_regression … every_symbol_carries_its_evidence`, plus the 13 stale renderer tests already queued. *2026-09-03, mark-memory commit:* the `four_tools` and `specific_kind` tests were stale assertions and are fixed (the full eight-tool set is pinned; the meaning sentence pins `route`). The two pipeline gates are real extractor drift - `bound-counter` is now flagged as a second writer, and `env_read` symbols carry no comparable evidence - and stay open.
