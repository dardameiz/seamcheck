...
// signal_map/css_tools/parse_css.mjs
...
process.stdin.on("end", () => {
  for (const filePath of buffer.split("\n").filter(Boolean)) {
    ...
    try {
      const root2 = postcss_default.parse(readFileSync(filePath, "utf8"), { from: filePath });
      root2.walkRules((rule2) => {
        record.selectors.push({ selector: rule2.selector, line: rule2.source?.start?.line ?? null });
      });
      root2.walkDecls((decl2) => {
        const line = decl2.source?.start?.line ?? null;
        if (decl2.prop.startsWith("--")) {
          record.tokenDefs.push({ name: decl2.prop, line });
        }
        for (const match of decl2.value.matchAll(VAR_USE)) {
          record.tokenUses.push({ name: match[1], line, fallback: Boolean(match[2]) });
        }
      });
      root2.walkAtRules("import", (rule2) => {
        record.imports.push({ params: rule2.params, line: rule2.source?.start?.line ?? null });
      });
    } catch (err) {
